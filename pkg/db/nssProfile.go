...
func InsertNssProfileTable(profileId string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to create SliceProfile: ", err)
	}
	return
}
...
func InsertNssaiTable(profileId, sst, sd string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to create SNssaiTable: ", err)
	}
	return
}
...
func DeleteNssaiTable(profileId string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to delete SNssaiTable (profileId: ", profileId+"): ", err)
	}
	return
}
...
func InsertPlmnTable(profileId, mnc, mcc string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to create PlmnTable: ", err)
	}
	return
}
...
func DeletePlmnTable(profileId string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to delete PlmnTable (profileId: ", profileId+"): ", err)
	}
	return
}
...
func DeletePerreqTable(profileId string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to delete PerReqTable (profileId: ", profileId+"): ", err)
	}
	return
}
...
func InsertPerreqTable(profileId string, atCapDl, atCapUl, edrDl, edrUl, oUserDsty int) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to create PerReqTable: ", err)
		logger.DbLog.Errorln("Exec: " + "INSERT INTO PERREQLISTS (SliceProfileId, ExpDataRateDl, ExpDataRateUl, AreaTrafficCapDl, AreaTrafficCapUl, OverallUserDensity) VALUES ( '" + profileId + "', " + strconv.Itoa(edrDl) + ", " + strconv.Itoa(edrUl) + ", " + strconv.Itoa(atCapDl) + ", " + strconv.Itoa(atCapUl) + ", " + strconv.Itoa(oUserDsty) + ")")
	}
	return
}
...
func DropNssProfile(profileId string) (err error) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to drop NssProfile table: ", err)
	}
	return
}
...
func GetNssProfile(profileId string) (err error, status int) {
	...
	if err != nil {
		logger.DbLog.Errorln("Failed to filter SliceProfileId from sliceProfileList table: ", err)
		status = 503
		return
	}
	...
	for rows.Next() {
		...
		err = rows.Scan(&s)
		if err != nil {
			logger.DbLog.Errorln("Failed at parsing SliceProfileId query: ", err)
		}
		status = s
		return
	}
